...
{\
    unsigned char *base_ptr = start_ptr;\
    while((start_ptr - base_ptr) != buffer_size) {\
    type = *start_ptr;\
    length = *(start_ptr + 1);\
    tlv_ptr = start_ptr + TLV_OVERHEAD_SIZE;\
    start_ptr += TLV_OVERHEAD_SIZE + length;\
...
prepare_tlv_buffer(uint32_t *total_buffer_size){
    ...
    *(emp_id + 2) = 52439;
    ...
    temp += TLV_OVERHEAD_SIZE + 24;

    *temp = IP_ADDRESS_TLV_CODE;
    *(temp + 1) = 64;

    uint8_t *ip_addr = (uint8_t *)(temp + TLV_OVERHEAD_SIZE);
    strncpy(ip_addr, "10.1.1.1", 16);
    strncpy(ip_addr+16, "10.1.1.2", 16);
    strncpy(ip_addr+32, "10.1.1.3", 16);
    strncpy(ip_addr+48, "10.1.1.4", 16);
